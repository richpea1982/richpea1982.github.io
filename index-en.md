...
* **Pragmatic Defensively Isolated Control Planes:** A cluster is only as stable as the infrastructure supporting it. I deliberately isolated my core network routing (OPNsense), backups (PBS), and automation engines onto a dedicated machine completely outside the cluster pool. This ensures I never suffer from a "chicken-and-egg" loop where a cluster failure brings down my deployment tools or core network access.
...
* **Infrastructure Core Node (pve1):** Deliberately left outside the main compute cluster. It runs a virtualized **OPNsense** firewall acting as the routing entry point for the entire network, a **Proxmox Backup Server (PBS)** instance, and my **automation control node** for provisioning and configuring the environment.
* **Storage Server (Bare-Metal NAS):** A dedicated bare-metal desktop PC acting as a high-capacity NAS, provisioned with a **6-drive ZFS RAID-Z2 array** (6 x 1TB HDDs). This layout provides a 2-disk fault tolerance layout for 4TB of usable data protection.

### 2. The Compute Tier (3-Node Hyperconverged Cluster)
* **Proxmox VE + Ceph Cluster:** Composed of 3 physical hypervisor nodes running an embedded **Ceph distributed storage pool** for stateful components. This backs two production WordPress sites: `petitsanglais`, a small site I built and maintain myself, and `hantaweb`, a full WooCommerce e-commerce store that I host and administer at the infrastructure level (design and development are handled by the site owner).
* **Containerized Microservices:** A 3-node **K3s Kubernetes cluster** with embedded Etcd, currently in active build-out — bootstrapping HA control-plane nodes and validating self-healing behaviour ahead of hosting stateless apps and internal platforms.

### 3. Network & Security Architecture
* **Zero-Trust Entry Point:** Inbound connections are handled via Cloudflare Tunnels with no open external ports. 
* **Micro-Segmentation:** Traffic is strictly segregated into dedicated management, DMZ, and internal VLANs. Within the Kubernetes cluster, **Calico CNI** network policies enforce strict pod isolation, while a **Traefik v3** ingress controller routes traffic through local **CrowdSec** bouncers for layer-7 HTTP protection.

---

## 📁 Deep Dive Into The Projects

* **[Infrastructure Overview](/en/infrastructure.html)** — Physical layout, backbone segregation, and hardware inventory.
* **[IaC & Automation](/en/iac-automation.html)** — Terraform modules, Ansible roles, and bootstrap orchestration pipelines.
* **[Network Architecture](/en/networking.html)** — OPNsense routing rules, VLAN configurations, and Zero-Trust tunneling.
* **[Services & Applications](/en/services.html)** — Kubernetes (K3s) layout, Traefik ingress routing, and database lifecycles.
* **[Security Model](/en/security.html)** — Threat modeling, CrowdSec log parsing, and secret injection.
* **[Backup & Disaster Recovery](/en/backup-strategy.html)** — 3-2-1 rule implementation, Proxmox Backup Server, and ZFS replication.
