...
## At a glance
- **Local proxmox-srv rollbacks:** Proxmox snapshots for fast VM/LXC recovery.  
- **Secondary proxmox backups:** PBS on Docker‑SRV stores Proxmox backups to an attached USB SSD.  
- **Docker-srv backups:** Restic makes copies on the OS SSD and the USB SSD.  
- **Off‑site DR:** Restic uploads encrypted snapshots proxmox-srv and docker-srvto cloud storage for disaster recovery.
```mermaid
flowchart LR
...
- **Operator:** Single maintainer (me). Keep encryption keys and credentials in ansible-vault.  
...
- **Weekly:** Perform cloud upload; verify integrity of one cloud snapshot.  
...
- **Secrets and keys:** Store encryption keys and cloud credentials in ansible-vault; rotate and version them during monthly maintenance.  
