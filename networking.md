...
| **VLAN 90** | `TAILNET` | `10.0.90.0/24` | Interface virtuelle dédiée au VPN maillé Tailscale (via le plugin `os-tailscale`). |
...
- **VLAN 20 (K3s) → VLAN 10 (NAS / S3) :** Autorisé uniquement vers l'IP spécifique du NAS (`10.0.10.15`) sur les ports de stockage `9000` (MinIO API) et `9001` (Console) pour la persistance et l'accès au state Terraform. Tout autre accès vers le VLAN 10 est rejeté.
...
- **Configuration UI (`VPN > Tailscale > Settings`) :**
  - **Advertise Subnet Routes :** Activé et configuré pour annoncer l'ensemble de nos supernets locaux (ex. `10.0.0.0/16` ou nos VLANs spécifiques `10.0.10.0/24`, `10.0.20.0/24`).
