...
- **Layered enforcement:** Traefik enforces routing and TLS; CrowdSec analyzes traffic; a bouncer applies decisions at Cloudflare.  
...
2. **Cloudflare Edge to Cloudflared Tunnel.** Traffic is forwarded over an outbound, encrypted tunnel to my homelab—no inbound ports required.  
...
4. **Security decision flow.** CrowdSec evaluates traffic patterns; Cloudflare Bouncer updates edge firewall rules to limit or block repeat offenders.  
...
- **Incident response:** Block malicious IPs at Cloudflare first via the Cloudflare Bouncer.  
